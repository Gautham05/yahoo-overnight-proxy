...
def fetch_overnight(ticker):
    try:
        url = f'https://finance.yahoo.com/quote/{ticker}/'
        r = session.get(url, timeout=15)
        html = r.text

        # data-testid is stable unlike class names
        match = re.search(r'data-testid="qsp-overnight-price"[^>]*>([\d.]+)<', html)
        if match:
            return ticker, {'overnightPrice': float(match.group(1))}
        else:
